<template>
  <div class="reg">
    <ul>
      ...
      <li class="wrapper border-bottom">
        <input type="password" id="sec_password" placeholder="请再次输入密码" required="required" class="inp" v-model="sec_password" />
        <span class="succTip">{{succInfo_3}}</span>
      </li>
    </ul>
...
export default {
  ...
  methods: {
    ...
    handleSubmit () {
      ...
      if (flag === true) {
        if (!(/^(?![^a-zA-Z]+$)(?!\\D+$).{8,16}$/.test(this.inputData[2]))) {
          this.errorInfo = '确认密码：至少包含一个数字和字母!'
          this.display()
          flag = false
        } else {
          if (this.inputData[2] !== this.inputData[1]) {
            this.errorInfo = '确认密码：密码不一致，请确认后重新输入!'
            this.display()
            flag = false
          } else {
            this.succInfo_3 = '✔'
